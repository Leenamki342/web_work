...
<%
	...
	/*
		[ 검색 키워드에 관련된 처리 ]
		- 검색 키워드가 파라미터로 넘어올수도 있고 안넘어올수도 있다.
	*/
	String keyword=request.getParameter("keyword");
	String condition=request.getParameter("condition");
	// 만약 키워드가 넘어오지 않는다면
	if(keyword==null){
		// 키워드와 검색 조건에 빈 문자열 입력
	...
	//startRowNum 과 endRowNum  을 Dto 객체에 담고
	...
	// 만약 검색 키워드가 넘어오면
	if(!keyword.equals("")){
		// 검색 조건이 무엇이냐에 따라 분리하기
		if(condition.equals("title_filename")){ // 제목 + 파일명 검색일 경우
			// 검색 키워드를 FileDto 에 담아서 전달
			dto.setTitle(keyword);
			dto.setOrgFileName(keyword);
			// 제목+파일명 검색일때 호출하는 메소드를 이용해서 목록 얻어오기
			list=FileDao.getInstance().getListTF(dto);
			// 제목+파일명 검색일때 호출하는 메소드를 이용해서 row 갯수를 얻어오기
		...
		}// 다른 검색조건을 추가하고 싶다면 아래 else if() 를 계속 추가하면 된다.
	}else{ // 검색 키워드가 넘어오지 않으면
		// 키워드가 없을때 호출하는 메소드를 이용해서 파일 목록을 얻어온다.
		list=FileDao.getInstance().getList(dto);
		// 키워드가 없을때 호출하는 메소드를 이용해서 전체 row 의 갯수를 얻어온다.
...
<div class="container">
	<nav>
		<ul class="breadcrumb">
			...
			<li class="breadcrumb-item active">피카츄 자료실</li>
		</ul>
	</nav>
